...
@app.route("/api/v1.0/total_vaccinations_per_hundred")
@cross_origin()
def total_vaccinations_per_hundred():
    conn_string = "host='localhost' dbname='covid_vaccine'\
    user='postgres' password='Juhi@123'"
    # use connect function to establish the connection
    conn = psycopg2.connect(conn_string)
    data = pd.read_sql('select total_vaccinations_per_hundred from vaccine2', conn)
    return data.to_json()
...
def people_fully_vaccinated_per_hundred():
    ...
    # use connect function to establish the connection
    conn = psycopg2.connect(conn_string)
    data = pd.read_sql('select people_fully_vaccinated_per_hundred from vaccine2', conn)
    return data.to_json()
